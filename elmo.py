...
with tf.name_scope("dnn"):
    e = tf.placeholder(tf.float32, shape=[None, 1024], name="elmo")
...
with tf.Session() as sess:
    ...
    e_train = sess.run(embed([t[0] for t in training]))
    e_dev = sess.run(embed([t[0] for t in dev]))
    e_test = sess.run(embed([t[0] for t in test]))
